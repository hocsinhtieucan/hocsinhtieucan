...
function setupEventListeners() {
    ...
    if (chatbotForm && chatbotInput && chatbotMessages) {
        chatbotForm.addEventListener('submit', async (e) => {
            ...
            chatbotInput.value = '';
            // Lưu lịch sử user
            saveChatHistory({ role: 'user', text: userMsg, html: userDiv.innerHTML });
            // Hiển thị trạng thái đang trả lời
            const botDiv = document.createElement('div');
            botDiv.className = 'chatbot-msg loading';
            botDiv.innerHTML = '<i class="fas fa-spinner fa-spin"></i>...';
            ...
            try {
                ...
                // Lưu lịch sử bot
                saveChatHistory({ role: 'bot', text: reply, html: botDiv.innerHTML });
            } catch (err) {
                botDiv.className = 'chatbot-msg bot';
                botDiv.textContent = 'Lỗi kết nối API.';
                saveChatHistory({ role: 'bot', text: 'Lỗi kết nối API.', html: 'Lỗi kết nối API.' });
            }
            chatbotMessages.scrollTop = chatbotMessages.scrollHeight;
...
// Format nội dung trả về từ DeepSeek cho dễ đọc
function formatBotReply(text) {
    if (!text) return '';
    // Thay **text** thành <b>text</b>
    let html = text.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');
    // Thay (text) thành <i>text</i>
    html = html.replace(/\(([^\)]+)\)/g, '<i>($1)</i>');
    // Nếu có xuống dòng, thay bằng <br>
    if (html.includes('\n')) {
        html = html.replace(/\n/g, '<br>');
    } else {
        // Nếu không có xuống dòng, tự động thêm <br> sau dấu chấm, hỏi, cảm thán
        html = html.replace(/([.!?])\s+/g, '$1<br>');
    }
    return html;
}
        });
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', debounce(() => {
        ...
        if (filteredPosts.length > 0) {
            renderSearchResults(filteredPosts, query);
    ...
    searchInput.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            navigateToPost(filteredPosts[selectedIndex].id);
    ...
    searchResults.addEventListener('click', (e) => {
        ...
        if (resultItem && resultItem.dataset.id) {
            navigateToPost(parseInt(resultItem.dataset.id));
        }
    });
...
const API_KEY = "sk-edfdf973a77c4a7eb09b064de968055c";

async function callDeepSeek(prompt) {
    const messages = [
        { role: "system", content: "Bắt buộc trả lời bằng tiếng việt, bắt buộc ngắn gọn khoảng 100 từ" },
